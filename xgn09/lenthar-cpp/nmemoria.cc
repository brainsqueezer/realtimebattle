...
void nMemoria::Store_Message(int mensaje, int par_1, char par_2[256])
{
	switch(par_1)
	{
		case UNKNOWN_MESSAGE:
			escritor->Print("WARNING - Mensaje Desconocido:");
			escritor->Print(par_2);
			break;

		case PROCESS_TIME_LOW:
			escritor->Print("WARNING - Tiempo de proceso lento:");
			escritor->Print(par_2);
			break;

		case MESSAGE_SENT_IN_ILLEGAL_STATE:
			escritor->Print("WARNING - Mensaje enviado en estado ilegal:");
			escritor->Print(par_2);	
			break;

		case OBSOLETE_KEYWORD:
			escritor->Print("WARNING - Comando obsoleto:");
			escritor->Print(par_2);			
			break;

		case NAME_NOT_GIVEN:
			escritor->Print("WARNING - Nombre no ofrecido:");
			escritor->Print(par_2);
			break;

		case COLOUR_NOT_GIVEN:
			escritor->Print("WARNING - Color no ofrecido:");
			escritor->Print(par_2);
			break;
